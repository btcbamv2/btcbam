...
def main():
    ...
    tmpdir = "%s/bitcoinbam_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and bitcoinbamd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    #Set env vars
    if "BITCOINBAMD" not in os.environ:
        os.environ["BITCOINBAMD"] = build_dir + '/src/bitcoinbamd' + exeext
        os.environ["BITCOINBAMCLI"] = build_dir + '/src/bitcoinbam-cli' + exeext
